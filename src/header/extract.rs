...
impl<'a> IndexedLogHeader<'a> {
    pub fn get_online_mode(&self) -> Option<bool> {
        let mode = self.text.get(self.index.online_mode?..)
    ...
    pub fn get_mc_folder_location(&self) -> Option<&'a str> {
        self.text.get(self.index.mc_folder_location?..)?.lines().skip(1).next()
    }

    pub fn get_java_path(&self) -> Option<&'a str> {
        self.text.get(self.index.java_path?..)?.lines().skip(1).next()
    }

    pub fn get_java_version(&self) -> Option<JavaVersionInfo> {
        // e.g. "1.8.0_202, using 64 (amd64) architecture, from Oracle Corporation."
        let java_version_line = self.text.get(self.index.java_version?..)?.lines().next()?.strip_prefix("Java is version ")?;
    ...
    pub fn get_hardware_info(&self) -> Option<String> {
        ...
        let str = self.text.get(java_version_index..kernel_driver_index)?;
    ...
    pub fn get_opengl_version(&self) -> Option<&'a str> {
        ...
        Some(self.text.get(self.index.opengl_version?..)?.lines().next()?.strip_prefix("OpenGL version string: ")?)
    }
    ...
    pub fn get_kernel_driver(&self) -> Option<&'a str> {
        ...
        Some(self.text.get(self.index.kernel_driver?..)?.lines().next()?.strip_prefix("Kernel driver in use: ")?)
    }

    pub fn get_main_class(&self) -> Option<&'a str> {
        Some(self.text.get(self.index.main_class?..)?.lines().skip(1).next()?.trim_ascii_start())
    }

    pub fn get_native_path(&self) -> Option<&'a str> {
        Some(self.text.get(self.index.native_path?..)?.lines().skip(1).next()?.trim_ascii_start())
    }
    ...
    pub fn get_traits(&self) -> Option<Vec<String>> {
        let mut lines = self.text.get(self.index.traits?..)?.lines().skip(1);
    ...
    fn get_libraries_section(&self, start_index: usize) -> Option<Vec<LibraryInfo>> {
        let mut lines = self.text.get(start_index..)?.lines().skip(1);
    ...
    pub fn get_mods(&self) -> Option<Vec<ModInfo>> {
        let mut lines = self.text.get(self.index.mods?..)?.lines().skip(1);
    ...
    pub fn get_params(&self) -> Option<&'a str> {
        Some(self.text.get(self.index.params?..)?.lines().skip(1).next()?.trim_ascii_start())
    }

    pub fn get_window_size(&self) -> Option<(usize, usize)> {
        // e.g. "Window size: 854 x 480"
        let size_line = self.text.get(self.index.window_size?..)?.lines().next()?.strip_prefix("Window size: ")?;
    ...
    pub fn get_java_arguments(&self) -> Option<Vec<String>> {
        ...
        let args_line = self.text.get(self.index.java_arguments?..)?.lines().skip(1).next()?.strip_prefix("[")?.strip_suffix("]")?;
    ...
    pub fn get_mc_process_id(&self) -> Option<usize> {
        let pid_line = self.text.get(self.index.mc_process_id?..)?.lines().next()?.strip_prefix("Minecraft process ID: ")?;
        Some(pid_line.parse().ok()?)
    }

    pub fn get_jvm_info(&self) -> Option<&'a str> {
        // e.g. "JVM info: Microsoft - 17.0.15 - 17.0.15 6-LTS"
        Some(self.text.get(self.index.jvm_info?..)?.lines().next()?.strip_prefix("JVM info: ")?.trim_ascii_start())
    }

    pub fn get_ipv4_preferred(&self) -> Option<bool> {
        let ipv4_line = self.text.get(self.index.jvm_info?..)?.lines().skip(1).next()?;
    ...
    pub fn get_current_time(&self) -> Option<&'a str> { // TODO consider parsing later
        ...
        Some(self.text.get(self.index.current_time?..)?.lines().next()?.strip_prefix("Current Time: ")?.trim_ascii_start())
    }
