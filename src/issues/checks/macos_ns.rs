...
fn macos_ns(text: &str) -> Option<Issue> {
    text.contains("Terminating app due to uncaught exception 'NSInternalInconsistencyException").then_some(Issue::MacOSNSInternal)
}
...
mod tests {
    ...
    fn matches_macos_ns() {
        ...
        let issue = macos_ns(&text).expect("Failed to determine issue");
