...
pub(crate) fn entrypoint_execution_errors(report: &CrashReport) -> Option<Issue> {
    // Could not execute entrypoint stage 'main' due to errors, provided by 'create' at 'com.simibubi.create.Create'!
    let msg = &report.stacktrace.first()?.message;
...
mod tests {
    ...
    fn find() {
        ...
        let issue = entrypoint_execution_errors(&report).expect("Failed to find issue");
